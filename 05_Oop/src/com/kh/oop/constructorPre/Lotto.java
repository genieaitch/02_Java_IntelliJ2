...
public class Lotto {
    ...
    public void randomNumber() {
        ...
        while (count < 6) {
            ...
            // Math.random() * 45 0~44 로또번호는 1부터 시작 +1 붙여주고 1~45출력될 수 있도록 정정
            ...
            //새로 생성된 번호가 이미 만들어진 번호와 새로 만들어진 번호가 중복인지 확인하기 위해 배열에서 이미 채워진 부분만 보면 되는 것
            for (int i = 0; i < count; i++){
                if(num == lotto[i]){
                    중복확인 = true;
                    break;//기존에 있는 수와 랜덤으로 나온 수가 동일한 숫자이기 때문에 다시 랜덤으로 숫자를 생성하라고 돌려보내기
                }
            }
            //중복이 없을경우 배열에 추가
            if(!중복확인){
