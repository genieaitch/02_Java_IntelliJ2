...
public class For문 {
    ...
    public void method1() {
        ...
        //조건식이 flase가 될 때까지 반복 실행
        }
        초기식 : 자료형 변수명 = 시작할 숫자
                 초기식에 들어갈 변수명은 i, abc, xyz 등 개발자가 원하는 변수명 작성
                 초기식에 평균적으로
                 int i = 0 을 작성하는 이유는 배열이나 리스트에서 인덱스(index) 값이 0으로 시작하기 때문에 int i=0; 초기값을 작성하게 됨
                 i는 index의 약자의 의미로 int i = 0;와 같은 초기식을 작성하기 시작
    ...
    public void method3() {
        ...
        for (int i = 2; i < 10; i++) {
            ...
            for (int j = 1; j < 10; j++) {
                System.out.println(i + " X " + j + " = " + i * j); //한 칸 줄바꿈해서 단이 변경될 때마다 한줄 공백 처리
            }
            System.out.println();
        }
    }
    ...
    public void method4() {
        ...
        자바 5 버전에서부터 도입된 반복문 **우리는 현재 java 11버전 사용 중
