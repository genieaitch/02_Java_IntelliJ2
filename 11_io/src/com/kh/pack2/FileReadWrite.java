...
public class FileReadWrite {
    ...
    public void method1(){
        System.getProperty("user.home");//c/users/user1 + /바탕화면
                            //    c     users   user1    바탕화면에서 ex.txt.만들기
        ...
        try { //파일 안에 글을 작성하는 건 문제가 많을 수 있기 때문, 예외상황을 감지하는 try-catch 사용
            FileWriter writer = new FileWriter(file);

            String content = "안녕하세요. 반갑습니다. 환영해요.";

            writer.write(content); //파일쓰기 기능으로 파일안에 글자를 쓸 기능 호출() 안에 작성할 내용 추가
    ...
    //FileWriter fw = new FieWriter("파일명", true) true 파일 이어쓰기 모드 활성화
    public void method2(){
        System.getProperty("user.home");
        File file = new File(System.getProperty("user.home")+"/Desktop/ex1.txt");

        // FieWriter를 이용해서 이어쓰기 진행
        try {
            FileWriter fw = new FileWriter(file, true); //파일이 존재할 경우 이어쓰기모드

            String content2 = "\n이어서 작성한 내용입니다."; //새로운 내용 추가
            fw.write(content2);
            fw.close(); //이어 작성 완료 후 파일 닫기 = 저장
    ...
    //만들어진 파일을 읽고 눈으로 어떤 내용이 작성되어있는지 확인
    ...
    public void method3(){
        ...
        try {
            ...
            BufferedReader br = new BufferedReader(file); //한 줄 단위로 읽는 성능좋은 부스터 붙임
            //String line = br.readLine();
            String line; //한줄씩 읽어오는 readLine()기능 설정
            //반복문을 이용해서 읽는 것이 끝날 때까지 계속 반복
            while ((line = br.readLine()) != null) { //null = 더 이상 읽을 게 없을 때까지
    ...
    public void method4(){
        ...
        try {
            ...
            while ((line = br.readLine()) != null) { //line이라는 공간에 한 줄의 글자들이 담기고
                System.out.println(line); //line이라는 변수명에 담겨진 글자를 출력한 후 다음 줄이 아예 없을 때까지 계속 출력을 진행하는 것
            }
