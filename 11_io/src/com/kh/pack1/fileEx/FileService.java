...
public class FileService {
    ...
    public void method1() {
        ...
        System.out.println("폴더가 존재하는지  : " + 폴더.exists());
    ...
    public void method2() {
        ...
        if (file.exists()) {
            ...
            try { //폴더만들기와 다르게 파일 만들기는 문제가 생길 가능성이 높기 때문에 try-catch나 thorow와 같은 에러 방지 구문을 추가해줘야함
                file.createNewFile();
            } catch (IOException e) {
                System.out.println("파일을 만드는 데 문제가 발생했습니다.");
            }
    ...
    public void method3() {
        ...
        // 왜냐하면 \n 줄바꿈이나 \특수기호 사용과 같은 자바에서 특수문자 규칙으로 오인하여 부딪힐 수 있다
        ...
        for (File f : files) { //일반 for문이 아니라 향상된 for문을 사용하는 이유
            ...
            //마지막 수정 날짜 : 1731554733199 → 1970년 1월 1일부터 지난 일자로 표기됨
            ...
            //날짜 형식을 문자열 형태로 변환
            String formatDate = sdf.format(date); // 1731554733199 → sdf 형식으로 변경해서 문자열로 전달
            String formatDate2 = sdf.format(f.lastModified());
            //Cannot format given Object as a Date 파일의 마지막 수정 날짜를 최대한 큰 공간으로 가져온 뒤 Date로 가져오기
            //가장 큰 공간 : long으로 가져와서 변화 → 왜냐하면 날짜의 끝은 제대로 담으라는 의미
            System.out.println("사람이 읽기 편한 날짜 형식으로 변경 : " + formatDate);
        }
    }
}