...
public class MapService {
    ...
    public void method1(){
        ...
        //우리가 작성한 순서와 관계없이 나열
        //key 스페이스바를 추가하면 스페이스바로 넣은 띄어쓰기 또한 하나의 글자로 인식
        //마치 중복돼서 출력 되는 것처럼 보임
        //KR=korea가 네 번째 보여질 것
        ...
        //4. V remove(K k) : Key가 일치하는 Entry 제거 ( K : V)
        //일치하는 키가 있다면 value, 없다면 null 반환
        나라들.remove("KR");
        나라들.remove("EU ");
        System.out.println("제거 후 나라들 출력 : " + 나라들);

        //만약 지우는 광경을 확인하길 원한다면 remove를 출력문 안에 작성해주면 됨
        ...
        나라들.clear();
        System.out.println("clear()로 모두 지운 후 안에 값이 들어있는지 확인 : " + 나라들.isEmpty());
    ...
    public void method2(){
        ...
        //향상된 for문 + Set
        for(String key : set){//set에 저장된 [집, 63빌딩, 학원, 롯데타워]를 순차적으로 key 저장
            //저장된 키에 해당하는 값을
    ...
    public void method3(){
        ...
        //[집=서울시 강남구, 63빌딩=서울시 영등포구, 학원=서울시 강남구, 롯데타워=서울시 송파구]
        //set이라고 된 변수명이 Map.Entry<String, String> 이라는 키 값 형태로 값을 유지하고 있기 때문에
        //set 리스트에 작성된 목록들을 key라는 변수명으로 하나씩 가져올 때 Map.Entry<String, String> 이 형태로 가져온다는 표기

        // entry = key-value 한쌍으로 들어있는 존재
