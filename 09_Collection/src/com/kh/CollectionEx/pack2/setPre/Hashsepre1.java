 ...
 */
public class Hashsepre1 {
    /*
    Set (집합, 주머니)
    - 기본적으로 순서를 유지하지 않음 (index가 없음 → get() 메서드도 없음)
    -  중복데이터 저장X (같은 값이 들어오면 덮어쓰기)
    ...
    */
    /*
    method1() 아무 값을 넣고 중복허용되는지 확인
    */
    public void method1() {
        HashSet set = new HashSet(); //자료형 제한없이 String int 개발자가 만든 자료형 모두 OK
        set.add("점심");
        set.add(100);
        set.add(true);
        set.add(3.14);
    ...
    public void method2() {
        ...
        //결과를 보기 위해서 System.out.print로 출력해서 확인해야함!
        ...
        System.out.println("set에 리스트 출력해서 목록이 비어있는지 확인 : " + set);
    }
    ...
    public void method3() {
        ...
        // ArrayList 숫자값을 제거할 때 Integer.valueOf(정수값)으로 안해도 됨 index가 아니라서
        // 값을 저장할 때 자리값 = 추가한 값
        ...
        intSet.add(500);
        intSet.add(40);
        System.out.println("모든 리스트 : " + intSet);

        intSet.remove(3000);
        System.out.println("3000 지운 리스트 : " + intSet);

        intSet.contains(500);
        System.out.println("500 있는지 확인 : " + intSet.contains(500));

        intSet.clear();
        System.out.println("모두 비웠는지 확인 : " + intSet.isEmpty());
        System.out.println("최종 리스트 : " + intSet);
    }

    // set을 향상된 for문 (=for-each) 출력하기
    // 일반 for문의 경우 index가 존재하지 않기 떄문에 사용 제한이 있음 그 대신 Iterator라는 반복문을 이용해서 출력
    public void method4() {
        ...
        //for-each문으로 HashSet 출력
        for (String s : strList) {
            System.out.println(s); //중복이 허용되지 않는 set 목록에서 하나씩 꺼낸다음 s 변수명에 값을 하나씩 담고 출력
        }
    }
    ...
    public void method5() {
        ...
        while (it.hasNext()) { //만약에 hasNext() = 다음 값이 존재한다면 true
            System.out.println(it.next()); //str 리스트에서 하나씩 꺼내올 것인데 꺼내온 것은 it에 저장될 것이고 next() 다음 값에 있는 것을 출력
            //it에는 현재 str에서 꺼낸온 값이 들어있음 값을 꺼냈으면 next 다음 값을 준비해서 it 넣는 것
        }

        /*
        Iterator = 컬렉션을 순차적으로 회전하는 객체
                   (HashSet 이외에도 ArrayList 등 Collection올 작성된 객체들에게 모두 사용 가능)
        ...
        it.next() = 현재 Iterator가 가리키고 있는 현재 값을 보여주고, 그 다음에 자동으로 다음 값으로 이동설정
        */
    }
    ...
    public void iterLotto() {
        ...
        for (int i = 0; lottoNumbers.size() < 6; i++) {
            ...
            //(Math.random() = 0.0~1.0 사이의 소수를 출력 0.0000001~0.000009
            //(Math.random() * 45 = 45.13243546453 앞에 (int)를 붙여주면 . 뒤에 있는 소수들이 버려짐
            //(Math.random() * 45) 소수점이 버려졌기 때문에 0부터 44까지의 숫자를 만듬
            //로또에는 0이라는 숫자가 없고 45까지 숫자가 존재하기 때문에 모든 수 뒤에 +1을 붙여줌으로써
            //0~45자리 랜덤 숫자를 만들 수 있음
            int num = (int) (Math.random() * 45) + 1;
            lottoNumbers.add(num); //만들어진 숫자를 중복허용되지 않는 set 담기
        }
