...
public class WhileEx {
    ...
    /*
    while은 조건이 참(true)인 동안 특정 코드 블록을 반복적으로 실행하는 제거 구현

    while(조건이 true일 때) {
        반복 실행될 코드
    }
    반복 횟수가 명확하지 않을 때 사용
    while(false)라면 내부 코드는 한 번도 실행되지 않기 떄문에 문제가 발생
    while(true)로 반드시 설정

    while문을 종료하는 방법은 ()안을 false로 만들거나 return 처리를 하는 방버
    ...
    while(isTrue) {
        ...
        //조건 1이 아닐 때 실행되는 코드
        isTrue = false //로 변경해서 shile문을 종료시킴
        }
     }
