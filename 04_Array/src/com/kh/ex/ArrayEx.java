...
public class ArrayEx {
    ...
    /*
     * 동일한 타입
     * 자바 배열은 int면 전부 다 int, String이면 모두 다 String, char 모두 다 char 같은걸로 통일 필수
     * */
    /*
     * 배열 선언, 할당, 초기화
     * */
    public void method1() {
        ...
        //배열 선언
        int[] arr;
        //Stack 영역에 int[] 자료형을 참조할 변수를 생성하고(주소 저장) 그 변수의 이름을 "arr"로 지정 / arr=array의 줄임말
        ...
        //*debug : 프로그램 수행 중 특정 시점에 어떤 값을 지니고 있는지 확인 → 코드 상태, 오류를 분석할 수 있음
    ...
    public void method2() {
         ...
         * 창고에 저장한 물건은 직접 정리를 해야지 나중에 데이터가 어디 있는지 알고 찾을 수 있음
         * */
    }

    /*
     * 정수 4개를 입력 받아 배열에 차례대로 대입 후 결과를 확인*/
    public void method3() {
        //배열 선언 + 할당
        //int arr = new int[4]; → 자바에게 값을 담을 공간이 배열형태임을 미리 알려주지 않으면 에러 발생
        //반드시 int나 변수명 앞 뒤에 [] 붙여줘야함, 단 int와 같은 자료형 앞은 붙이기X
        /*
        기본 자료형에 [] 붙었기 때문에 참조형 변수가 되는 것
        int arr = new int[4]; (X)
        []int arr = new int[4]; (X)
        int[] arr = new int[4]; (O)
        int []arr = new int[4]; (O)
    ...
    public void method4() {
        ...
        System.out.println(arr); //[I@5dfcfece 와 같은 주소 값 보임 →
        // [ 배열 시작 뜻 I 정수형이라느 뒤는 배열부분명칭 16진수 표기
        ...
        //index값이 0~4 까지 있기 때문에 없는 자리를 찾는다는 에러 발생
        for (int i = 0; i < arr.length; i++) {}
