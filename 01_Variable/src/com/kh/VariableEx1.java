 ...
 * 폴더명을 작성할 경우 .은 폴더안에 폴더로 들어간다는 의미이기 때문에
 * com c.o.m .으로 구분지어서 작성하지 않음
 * c.o.m → c라는 폴더 안에 o라는 폴더 안에 m이라는 폴더에 만들겠다는 표기
 */
...
public class VariableEx1 {
    public static void main(String[] args) {
         ...
         * byte	(1byte)		숫자뒤에 b B
         * short(2byte)		숫자뒤에 sh
         * int	(4byte)		*정수 기본 형태, 제일 많이 사용*
         * long (8byte)		숫자뒤에 L ㅣ
         *
         * [실수형] - 숫자 뒤에 소수점으로 실수 표현 가능
         * float (4byte)	숫자 뒤에 f를 붙여줘야함
        ...
        System.out.println("진실 혹은 거짓 =>" + 진실혹은거짓);

        double 실수1번; //메모리에 double 저장 공간 8byte 할당하고 그 공간을 실수 1번으로 부르겠다.
        ...
        String 문자열2번 = "필수로 맨 앞 S는 대문자 표기 보라색으로 글자색 변화X";

        System.out.println("변수명에 노란색 밑줄은 변수명을 선언하고+"
                + "아래에서 사용하지 않은 변수명이라는 표기");

        /*
         * boolean이나 char String 단독으로 예약어가 존재하기 때문에 상관없음
         * 예약어(자료형, 참조형) 변수명 = 리터럴;
         * [정수형]
         * 정수형 기본적으로 리터럴이 int로 설정
         * int(4byte) 안에서 int보다 byte 크기가 작은 short나 byte로 리터럴 변경할 경우 자동으로 타입 변환이 일어나고, 값이 해당 타입의 범위를 초과하면 오류 발생
         * 예)
         * byte - 128 ~ 127
         *
         * byte  자료형 = int
         ...
         * int는 위에서 작성한 숫자 범위를 초과하기 때문에 초과하는 숫자를 담기 위해서는 뒤에 L이나 l 로 작성해줘야함
         *
         * long 	변수명 = long
         * long 	롱2번 = 10000000000L; //long 사용 long 리터럴은 L 필수로 뒤에 와야함
         *
         * [실수형]
         * 실수형 리터럴은 기본적으로 double 타입으로 설정됨
         * 왜냐하면 float은 대략 소수점아래 7번째자리까지만 표기가 가능
         *
         * 나머지 8자리부터 버리기 때문에 안전하게 가져올 수 있도록 double 기본으로 사용
         * 소수점아래 소수들을 나타낼 때 8자리 이상이 아닌 경우 숫자 뒤에 f를 붙여서 절반공간의 크기만 사용하겠다 표기
         *
         * 자료형   변수명 = 리터럴
         * double 변수명 = double
         * double 더블1번 = 3.14;
         ...
         * 플롯1번의 공간의 크기를 float으로 주고 실수 뒤에 f를 붙여주지 않으면 실수는 기본리터럴형인 double로 인식되어 공간보다 큰 메모리가 온다고 컴퓨터는 인식하고 오류 발생하기 때문에 앞에 예약어를 float으로 작성했다면 소수 뒤에는 f를 붙여주는 ㄴ것
         *
         * L이나 f같은 리터럴 표기법은 컴퓨터가 자료형을 구분하기 위한 기호일 뿐 출력시 보여지지 않는다
         */

        char 문자1번 = '가';//숫자로 44032 이름으로 저장
        char 문자2번 = 'a';//숫자로 97 이름으로 저장
        //char 문자3번 = "A"; char문자에는 반드시 '' 감싸기!
